//! Live serving of stax samples over a vox WebSocket RPC service.
//!
...
impl LiveSink for LiveSinkImpl {
    fn on_sample(&self, event: &SampleEvent) {
        ...
        // already logged independently by staxd-client).
...
pub async fn start(addr: &str) -> Result<(LiveSinkImpl, tokio::task::JoinHandle<()>)> {
    ...
    tracing::info!("stax-live listening on ws://{}", local);
    eprintln!("stax-live: listening on ws://{}", local);
