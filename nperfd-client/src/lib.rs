...
//! [`Pipeline`] in `nerf-mac-kperf-parse` — the same parser, off-CPU
...
use nerf_mac_kperf_sys::kdebug::{self, KdBuf, DBG_MACH, DBG_MACH_SCHED, DBG_PERF};
...
pub async fn drive_session<S: SampleSink>(
    ...
    let shared_cache: Option<Arc<nperf_mac_shared_cache::SharedCache>> =
        nperf_mac_shared_cache::SharedCache::for_host().map(Arc::new);
