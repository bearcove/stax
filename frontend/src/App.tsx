...
/// and any class the theme doesn't cover fall back to a bare <span>
/// (inherited foreground). The annotation/disassembly wire used to ship
/// pre-rendered HTML (`.html`); it now ships structured `tokens`, which
/// we render client-side — same look, no `dangerouslySetInnerHTML`.
const TOKEN_EL: Partial<Record<TokenClass["tag"], string>> = {
  Keyword: "a-k",
  Function: "a-f",
  String: "a-s",
  Comment: "a-c",
...
/// Render a token run into coloured inline elements. Stable keys: a
/// disassembled line / source snippet is re-rendered wholesale on each
/// update, so positional keys are fine.
function renderTokens(tokens: Token[]): ReactNode[] {
  return tokens.map((t, i) =>
    createElement(TOKEN_EL[t.kind.tag] ?? "span", { key: i }, t.text),
  );
}
                ...
                if (sh) {
                  ...
                  rows.push(
                    ...
                    >
                      ...
                      <td className="src-snip">
                        {sh.tokens.length > 0
                          ? renderTokens(sh.tokens)
                          : "(source not on disk)"}
                      </td>
                    </tr>,
                  );
