...
//!
//! Pairs with `stax-mac-kperf-sys`, which is the privileged half
//! (kperf framework calls + KERN_KDEBUG sysctl). This crate consumes
//! the `KdBuf` record stream that `-sys` produces and turns it into
//! the higher-level events the rest of stax works with.
//!
//! The split exists so that, when the eventual `staxd` daemon
