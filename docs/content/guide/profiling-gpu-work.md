...
- **`stax diagnose`** — target ingest counters: batches, recorded/dropped
  ...
  drops, why origins did not link (`bad_tid`, `no_thread`, `no_stack`,
  `too_far`), and the min/avg/max PET sample distance for linked or too-far
  origins. Use this when spans are missing or CPU-stack attribution is missing.
